...
//    private HttpSession session;
     ...
     * 大数据量导出静态变量
     */
    public static int EXCEL_MAX_CNT = 50000; //每个excel文件多少条数据
    public static int SHEET_MAX_CNT = 50000; //每个sheet多少条数据
...
//    }
     ...
     * @throws Exception
     */
	public HSSFWorkbook writeSheetNoMerge(HSSFWorkbook wb, String title,
    ...
    public void exportToExcel(String title, String creator, TableData tableData)
        ...
        wb = writeSheetNoMerge(wb,title,styles,creator,tableData);//写入工作表
    ...
    public void exportToExcel(ZipOutputStream zout, ExcelBean bean)throws Exception {
        ...
        for (int i = 1; i <= excel_num; i++) {
            ...
            for(int j = 1; j <= sheet_num; j++){
                ...
                _end = j * SHEET_MAX_CNT;
                _end = _end>sublist.size()?sublist.size():_end;
                System.out.println("正在导出第"+j+"个sheet：第"+(start+_start+1)+"~"+(start+_end)+"条记录");
                ...
                td.setSheetTitle("第"+(start+_start+1)+"~"+(start+_end)+"条记录");
                tds.add(td);
            }
            ...
            String sFileName = bean.getTitle() + "(" + (start+1) + "~" + (end) + ")" + ".xls";
//          fileNames = java.net.URLEncoder.encode(sFileName, "UTF-8");// 处理中文文件名的问题
//          sFileName = new String(sFileName.getBytes("UTF-8"), "GBK"); // 处理中文文件名的问题

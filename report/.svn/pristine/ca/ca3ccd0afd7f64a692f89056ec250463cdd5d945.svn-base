...
<script type="text/javascript">
	...
	$(function() {
		...
		srchForm = $("#srchForm");		
		sy.createDatagrid($("#returnData"),srchForm);
	});
	function rightMenu(){commRightMenu('returnData');}
...
function importExcel() {
		...
		$("#hiddenWin").html("<iframe src='importExcelForRetu' style='overflow:hidden;width:97.9%;height:97.9%;'></iframe>")
		$("#hiddenWin").window('open');
	}
	
	function deleteBySerialNo() {
		var selectRow = $("#returnData").datagrid('getSelected');
		if (null != selectRow) {
			if(confirm("确定要删除此记录吗?")) {
				$.ajax({
					type: "POST",
					url : 'retuDataDelete?id='+selectRow.id,
					dataType : "json",
					success : function(msg){
						if ("1" == msg.data) {
							alert("删除成功");
							$('#returnData').datagrid('load');
						} else if ("0" == msg.data) {
							alert("删除失败");						
						}
					}
	...
	function importDelExcel() {
		...
		$("#hiddenWin").html("<iframe src='importDelExcelForRetu' style='overflow:hidden;width:97.9%;height:97.9%;'></iframe>")
		$("#hiddenWin").window('open');
...
<body class="easyui-layout" style="width:100%;height:101%;" onload="comeIn()"
	 ...
	 <!-- 查询表单 -->
	 	<div class="Search-box" region="north" title="查询条件" style="height: 110px">
			<form id="srchForm" class="frmSearch" action="" autocomplete='off'>
				<input type="hidden" name="bean" value="returnData">
				...
				<table class="tableForm datagrid-toolbar"  align="center"  >
					...
					<tr>
						<td align="center" colspan="4">
							<a href="javascript:void(0)" class="easyui-linkbutton"onclick="importDelExcel()" id="importDelExcel">导入需要要删除的数据</a>&nbsp;&nbsp;&nbsp;
	...
	<!-- 数据表格 -->		
		<div region="center">
			<table id="returnData" class="easy-table">
				<thead>   
			       <tr>
		...
		<div id="menu" class="easyui-menu" style="width: 120px; display: none;">
			...
			<div id="returnData_do_delete" onclick="deleteBySerialNo()">删除</div>
		</div>	

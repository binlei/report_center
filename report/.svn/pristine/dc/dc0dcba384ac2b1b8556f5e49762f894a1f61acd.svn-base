...
<script type="text/javascript">
	function load() {
		if ("" != '${sb}') {
			...
			alert('${serialNo}'.replace("aaaaa", "序列号为[").replace("bbbbbb","]的发货日期错误，请核对后再导入！"));
		}
...
<body onload="load()">
	<form id="file_upload_id" name="file_upload_name" action="resolveExcelForDeli" method="post" enctype="multipart/form-data">
		<div>
			请选择文件：<input type="file" name="excelFile"/>
		</div>
		<div>
<!-- 			<input type="submit" value="上传" class="easyui-linkbutton" onclick="aaa()" id="aaa"/> -->

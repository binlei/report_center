...
<script type="text/javascript">
	...
	$(function() {
		...
		srchForm = $("#srchForm");		
		sy.createDatagrid($("#salesNoInDeliData"),srchForm);
	});
	function rightMenu(){commRightMenu('salesNoInDeliData');}
 
	function deleteBySerialNo() {
		var selectRow = $("#salesNoInDeliData").datagrid('getSelected');
		if (null != selectRow) {
			if(confirm("确定要删除此记录吗?")) {
				$.ajax({
					type:"POST",
					url : 'salesNoInDeliDataDelete?id='+selectRow.id,
					dataType : "json",
					success : function(msg){
						if ("1" == msg.data) {
							alert("删除成功");
							$('#salesNoInDeliData').datagrid('load');
						} else if ("0" == msg.data) {
							alert("删除失败");						
						}
					}
	...
	function importDelExcel() {
		...
		$("#hiddenWin").html("<iframe src='salesNoInDeliMoreDele' style='overflow:hidden;width:97.9%;height:97.9%;'></iframe>")
	 ...
	 <!-- 查询表单 -->
	 	<div class="Search-box" region="north" title="查询条件" >
			<form id="srchForm" class="frmSearch" action="" autocomplete='off'>
				<input type="hidden" name="bean" value="salesNoInDeliData">
					...
					<tr>
						...
						<td>
							<select id="extendProp1" name="extendProp1" style="width: 150px; ">
								...
								<option value="已销售未发货">已销售未发货</option>
							</select>
						</td>
		...
		<div id="menu" class="easyui-menu" style="width: 120px; display: none;">
			...
			<div id="salesNoInDeliData_do_delete" onclick="deleteBySerialNo()">删除</div>
		</div>
